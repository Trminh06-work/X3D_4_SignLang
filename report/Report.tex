...
\begin{abstract}
    ...
    We fine–tune three \texttt{Kinetics–400}–pretrained variants on the \texttt{AUTSL} dataset \cite{sincan2020autsl} of $226$ Turkish signs, at sixtwo depths, increasingly freeze from 0 block, i.e. the whole network to 5 blocks, i.e. only classifier head is trained.
    \textcolor{red}{State the headline top--1 for the best configuration, the gap between the two fine--tuning depths, and the cost in GFLOPs at which it was obtained.} \textcolor{red}{Close with the one sentence a reader should take away about the accuracy--cost trade--off on this task.}
\end{abstract}
