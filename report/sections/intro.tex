...
Another application is early criminal activity recognition from surveillance cameras in real-time manner \cite{LIM2023}, and other applications can be found in \cite{HEO202632, KongFu2022}.
...
Nonetheless, there are non-negligible obstacles associated with Action Recognition applications, and the above-mentioned models cannot overcome all of these. As surveyed in \cite{HEO202632,KongFu2022}, one action is likely performed in various ways by different people,
...
Importantly, Computer Vision applications are mostly deployed to edge devices whose computing capacities are limited while demanding low latency and real–time inference accuracy \cite{MoViNets, DeepRT}.
...
\begin{itemize}
    ...
    \item Benchmark different fine-tuning approaches across different \texttt{X3D} variants on \texttt{AUTSL} dataset.
    \item A quantitative result on performance of \texttt{X3D} on isolated sign language recognition tasks.
    \item Evaluate \texttt{AUTSL}–trained \texttt{X3D} on our hand–crafted dataset.
\end{itemize}
